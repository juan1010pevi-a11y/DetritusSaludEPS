...
import { getPqrsById } from '../../services/api';
...
export default function EstadoPqrsd() {
  ...
  const handleSubmit = async event => {
    ...
    try {
      const data = await getPqrsById(form.get('caseNumber'));
      setRequest(data.items?.[0] || null);
      setStatus(data.items?.length ? 'Solicitud encontrada.' : 'No encontramos una solicitud con ese radicado.');
    } catch {
      setRequest(null);
      setStatus('No fue posible consultar el estado de la solicitud.');
    }
  };
      ...
      <section className="tramite-content">
        <div className="container">
          ...
          <form className="info-card tramite-form pqrs-form" onSubmit={handleSubmit}>
            ...
            <div className="form-group"><label htmlFor="case-number">Número de radicado</label><input id="case-number" name="caseNumber" type="text" placeholder="PQRS-2026-789" required /></div>
            ...
            {request && <p className="form-status">Estado: {request.status}<br />{request.description}</p>}
