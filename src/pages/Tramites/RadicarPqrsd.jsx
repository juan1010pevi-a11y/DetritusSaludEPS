...
import { createPqrs } from '../../services/api';
...
export default function RadicarPqrsd() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const data = await createPqrs({
        type: form.get('type'),
        description: `${form.get('subject')}: ${form.get('description')}`,
        affiliateId: form.get('document'),
      });
      setStatus(`Solicitud recibida. Tu número de radicado es ${data.request.id}.`);
    } catch {
      setStatus('No fue posible radicar la solicitud. Verifica que PQRS Service esté activo.');
    }
  };
      ...
      <section className="content-section">
        <div className="container pqrs-submit-layout">
          <form className="info-card pqrs-submit-form" onSubmit={handleSubmit}>
            ...
            <div className="form-row"><div className="form-group"><label htmlFor="pqrs-type">Tipo de solicitud</label><select id="pqrs-type" name="type" defaultValue="" required><option value="" disabled>Selecciona una opción</option><option>Petición</option><option>Queja</option><option>Reclamo</option><option>Sugerencia</option><option>Denuncia</option></select></div><div className="form-group"><label htmlFor="pqrs-subject">Asunto</label><input id="pqrs-subject" name="subject" type="text" placeholder="Motivo de tu solicitud" required /></div></div>
            <div className="form-row"><div className="form-group"><label htmlFor="pqrs-name">Nombre completo</label><input id="pqrs-name" name="name" type="text" placeholder="Tu nombre" required /></div><div className="form-group"><label htmlFor="pqrs-document">Documento</label><input id="pqrs-document" name="document" type="text" placeholder="1020304050" required /></div></div>
            ...
            <div className="form-group"><label htmlFor="pqrs-description">Describe tu solicitud</label><textarea id="pqrs-description" name="description" placeholder="Escribe los detalles de tu caso..." required /></div>
